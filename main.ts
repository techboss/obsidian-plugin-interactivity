            ...
            editorCallback: (editor: Editor, view: MarkdownView) => {
                ...
                if (typeof this.allSubprocesses[file] !== "undefined") {
                    function routine(that: InteractivityPlugin, byEnter: boolean, ch: number) {
                        ...
                        // NEW: Always collect the full selection first, handling single or multi-line
                        let selection = editor.getSelection();
                        let is_multi = (selection.match(/\n/gm) || []).length > 0;
                        let lines: string[] = [];
                        let first_line = '';
                        let line = editor.getCursor().line; // For later setCursor
                        if (selection.length == 0) {
                            selection = editor.getLine(line);
                            first_line = selection;
                            lines = [selection];
                        } else if (is_multi) {
                        ...
                        // NEW: Check shortcuts on the first_line of the (possibly multi-line) selection
                        let command = '';
                        let maxLen = 0;
                        for (const x of that.settings.shortcuts.split('\n')) {
                            let m = x.match(/(.*?)\s*->\s*(.*)/);
                            if (m && m.length > 2 && maxLen < m[1].length && first_line.indexOf(m[1]) == 0) {
                                // NEW: Use the entire rest of the selection as param (supports multi-line)
                                let param = selection.slice(m[1].length).trim();
                                // NEW: Escape param as Python triple-quoted string to handle newlines safely
                                // Escape any internal """ to avoid syntax errors
                                param = param.replace(/"""/g, '\\"""');
                                let escaped_param = '"""' + param + '"""';
                                command = m[2].replace(/##param##/g, escaped_param);
                                maxLen = m[1].length;
                            }
                        }

                        // If no shortcut matched, fall back to raw selection
                        if (command === '') {
                            selection = lines.join('\n');
                        } else {
                            selection = command;
